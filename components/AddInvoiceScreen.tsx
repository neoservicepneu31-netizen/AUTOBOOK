  ...
  const performAnalysis = async (base64: string, mime: string) => {
    ...
    try {
      ...
      if (result) {
        ...
        throw new Error("Aucun résultat exploitable");
      }
    } catch (error: any) {
      console.error("Scan error details:", error);
      setAnalysisError("L'IA n'a pas pu lire le document. Vérifiez la luminosité ou remplissez manuellement.");
    } finally {
    ...
    <div className="h-[100dvh] bg-nsp-bg flex flex-col w-full absolute inset-0 z-50 overflow-hidden">
      ...
      <div className="flex-1 p-4 overflow-y-auto pb-32">
        ...
        <div className="mb-6">
          ...
          >
            {imagePreview && <img src={imagePreview} className="absolute inset-0 w-full h-full object-cover opacity-30 blur-[1px]" />}
            ...
            <div className="z-10 flex flex-col items-center gap-3">
              {isAnalyzing ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-[11px] font-black text-white uppercase tracking-[0.3em] animate-pulse">Extraction en cours...</span>
                </div>
              ) : isSuccess ? (
                ...
                <>
                  ...
                  <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Prendre en Photo ou Importer</span>
                </>
              )}
            </div>
          </div>
          ...
          {finalBase64 && !isAnalyzing && (
            ...
            >
              <RefreshCw size={14} className={isAnalyzing ? 'animate-spin' : ''} /> {isSuccess ? 'Relancer le scan' : "Relancer l'analyse IA"}
            </button>
          )}
        </div>

        {analysisError && (
           <div className="mb-6 bg-red-950/40 border border-red-500/30 p-5 rounded-[2rem] flex items-center gap-4 text-red-200 animate-slide-up">
      ...
      {/* Input de type file acceptant l'appareil photo nativement */}
      <input 
        type="file" 
        ref={fileInputRef} 
        className="hidden" 
