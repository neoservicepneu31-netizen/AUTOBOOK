...
// Fix: Import required functions directly from node:process to avoid property access issues on the process object
import { cwd } from 'node:process';

export default defineConfig(({ mode }) => {
  // Charge les variables .env en local + les variables d'environnement Vercel
  // Fix: Use the imported cwd() function to reliably get the current working directory
